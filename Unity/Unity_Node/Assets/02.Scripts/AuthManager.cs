...
public class AuthManager : MonoBehaviour
{
    ...
    private const string ACCESS_TOKEN_PREES_KEY = "AccessToekn";
    private const string REFRESH_TOEKN_PRESS_KEY = "RefreshToken";
    ...
    {
        accessToken = PlayerPrefs.GetString(ACCESS_TOKEN_PREES_KEY, "");
        refreshToken = PlayerPrefs.GetString(REFRESH_TOEKN_PRESS_KEY, "");
    ...
    private void SaveTokenToPrefs(string accessToekn, string refreshToken, DateTime expiryTime)
    {
        PlayerPrefs.SetString(ACCESS_TOKEN_PREES_KEY, accessToekn);
        PlayerPrefs.SetString(REFRESH_TOEKN_PRESS_KEY , refreshToken);
    ...
    public IEnumerator Logout()
    {
        ...
        using (UnityWebRequest www = UnityWebRequest.PostWwwForm($"{SERVER_URL}/logout", "POST"))
        {
            ...
            else
            {
                ...
                PlayerPrefs.DeleteKey(ACCESS_TOKEN_PREES_KEY);
                PlayerPrefs.DeleteKey(REFRESH_TOEKN_PRESS_KEY);
    ...
    public IEnumerator RefreshToken()
    {
        ...
        using (UnityWebRequest www = UnityWebRequest.PostWwwForm($"{SERVER_URL}/token", "POST"))
        {
            ...
            {
                Debug.LogError($"Registration Error:{www.error}");
                yield return Login("username", "password");     //실구현에서는 저장된 사요자 정보를 사용
            }
            else
            {
